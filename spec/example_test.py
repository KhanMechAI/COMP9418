...
from src.solution import get_action
...
class SmartBuildingSimulatorExample:
    def __init__(self):
        ...
        self.room_occupancy = dict([(room, 0) for room in
                                    ['r1', 'r2', 'r3', 'r4', 'r5', 'r6', 'r7', 'r8', 'r9', 'r10', 'r11', 'r12', 'r13',
                                     'r14', 'r15', 'r16', 'r17', 'r18', 'r19', 'r20', 'r21', 'r22', 'r23', 'r24', 'r25',
                                     'r26', 'r27', 'r28', 'r29', 'r30', 'r31', 'r32', 'r33', 'r34', 'r35', 'c1', 'c2',
                                     'c3', 'c4', 'o1', 'outside']])
    ...
    def timestep(self, actions_dict=None):
        ...
        # increment time
        self.current_time = (
                datetime.datetime.combine(datetime.date.today(), self.current_time) + self.time_step).time()
...
for day in range(num_days):
    ...
    all_cost.append(simulator.cost)
    all_time.append((datetime.datetime.now() - start_t).total_seconds())
    result_string = (
        f"Day {day+1}\n"
        f"Total cost for the day: {simulator.cost} cents\n"
        f"Time taken: {all_time[-1]}s\n\n"
    )

    print(result_string)

    res_path = Path(__file__).parent / "results.txt"
    with open(res_path, "a+") as res:
        res.write(result_string)

all_result_string = (
    f"Mean Cost {np.mean(all_cost)}\n"
    f"Mean Time {np.mean(all_time)}"
)

print(all_result_string)